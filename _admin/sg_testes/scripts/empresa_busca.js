    ...
    function(d) {
        ...
        if (empresas.length == 0) {
            ...
            for (i = 0; i < empresas.length; i++) {
                var e = '<tr class="c">\n\
                            <td class="c">\n\
                               <a href="javascript:void(0)" onclick="javascript:mostraEmpresa(' + empresas[i].id + ')">\n\
                                  <b>' + empresas[i].nome + '</b>\n\
                               </a>\n\
                               <br />' +
                        empresas[i].html + '</td>' +
                        '<td class="c">\n\
                               <a href="javascript:void(0)" onclick="javascript:mostraFunc(' + empresas[i].id + ')">\n\
                                  [ver/gerenciar funcion&aacute;rios]\n\
                               </a>\n\
                            </td>';
                ...
                if (d.perm.editarEmpresa == '1')
                    e += '<td class="c">\n\
                            <a href="javascript:void(0)" onclick="javascript:editEmpresa(' + empresas[i].id + ')">\n\
                               [editar empresa]\n\
                            </a>\n\
                          </td>';
                e += '</tr>';
...
function deletaEmpresa(id) {
    ...
    var r = confirm("Deseja excluir a empresa?");
    if (r == true)
    {
        $.ajax({
            type: "POST",
            ...
            data: {empresaID: id, acao: "delEmpresa"},
            cache: false,
            dataType: "json",
            success: function(d) {
                alert("A empresa foi deletada");
                //alert(d);
                reload();
            }
        });
...
function mostraEmpresa(i) {
    ...
    $.ajax({
        ...
        url: "empresa.php",
        //data: {empresaID: $("#empresaID").val()},
        data: {acao: "PermdelEmpre"},
        cache: false,
        dataType: "json",
        success: function(d) {
            //valida recebe 1(caso tenha permissão) ou 2(caso não tenha permissão) de deletar uma empresa
            $valida = d; // d = valor retornado por echo da validação em "PermdelEmpre"
            ...
            function(d) {
                ...
                // se a valida for igual a 1, o usuário tem permissão para deletar empresa    
                if ($valida === 1) {

                    //tem permissão para deletar empresa
                    ...
                    // não tem permissão para deletar empresa
                    $("#empresaDetDialog").dialog({
                        title: "Detalhes da empresa " + d[0].nome,
                        autoOpen: true,
                        height: 400,
